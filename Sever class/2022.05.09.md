...
2. 명령어
    1) API 제공
        ...
        * _mm_lfence: Load 펜스, 해당 펜스 기준 이전 Load가 모두 처리가 되었음을 보장하는 것이다. 사실상 x86에서는 의미가 없는 펜스이다.
        * _mm_sfence: Store 펜스, 해당 펜스 기준 이전 Store를 모두 처리하는 것을 보장한다. 주의할 점은 x86에서 펜스 이후의 Load가 펜스 이전 Store를 제끼고 먼저 수행될 수 있
