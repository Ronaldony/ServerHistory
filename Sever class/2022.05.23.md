...
4. 동작과 비동기 입출력
    ...
    2) AcceptEx 와 ConnectEx 함수가 비동기 입출력을 지원한다. 그러나 불필요하기 때문에 우리의 사용 대상이 아니다.
...
1. WSASend
    ...
    3) 반환 값
        * WSA_IO_PENDING라는 오류 코드는 Non-block 소켓의 오류 코드인 WSAEWOULDBLOCK과 매칭된다.
2. WSARecv
    1) WSASend와 유사하다.
3. WSAOVERLAPPED 구조체
    1) 구조체
        ...
        * hEvent에 객체를 입력할 때 WSACreateEvent() 함수를 호출하여 입력한다. 이는 소켓 전용으로 사용하기위해 포장을 한 함수이다. 우리가 일반적으로 사용하는 커널 이벤트 오브젝트와 같다
    2) WSAOVERLAPPED와 그냥 OVERLAPPED 구조체(파일 사용 시)는 완전히 똑같은 구조체이다.
4. WSAWaitForMultipleEvents: WaitForMultipleObjects 기능과 동일
5. WSAGetOverlappedResult
    1) 구조
        <pre><code>
...
4. 우리는 Event 방식은 사용하지 않는다.
    1) 이유1: 작업(Send, Recv)당 Overlapped 구조체와 Event 객체가
    2) 이유2: Send와 Recv 작업 시 반환되는 바이트 개수가 
5. 모든 Overlapped IO는 Polling 방식이 아닌 Event 방식으로 처리가 된다. 이 말은 작업이 완료되면 알람과 같이 알려주는 형태라는 것을 의미한다.
...
2. 예제 분석
    1) WSARecv
    2) 더미 이벤트를 생성한 이유: Accept로 새로 연결된 세션에 대한 정보를 갱신하기 위한 이벤트 객체
...
# 기타 키워드
## 윈도우 시스템 프로그래밍 숙제
1. 예제1: 뒷부분에서 Overlap을 사용한 파일 입출력 시 문제점에 대한 이야기가 나온다. 이때 이 문제점과 동작 방식이 어떻게 되는지 파악하고 소켓과 어떤 차이점이 있는지 직접 테스트하여 파악해보기
