...
2. 윈속사용 시 단점
    ...
    2) 서로 다른 바이트 정렬, 데이터 처리 단위(32bit, 64bit)를 가진 호스트끼리 통신 시, 응용 프로그램 수준에서 데이터 변환을 처리해야 한다.

### 윈도우 소켓 구조
1. 윈속이 지원하는 대부분의 기능은 WS2_32.DLL로 제공된다. WINSOCK.DLL이나 WSOCK32.DLL은 궁극적으로 WS_2.DLL의 기능을 사용하게 된다.
2. Windows는 윈속 응용 프로그램을 위해 다음과 같은 구조를 띈다.
     * <img width=600 src="https://user-images.githubusercontent.com/95362065/150951135-c38f4008-861b-48f5-849f-0a00450d7682.jpg">

## 연습문제
### 네트워크 연결 장치
1. 리피터: OSI L1 계층으로 UTP 등의 케이블 신호를 증폭시켜주는 장치
2. 브리지: OSI L2 스위치로 들어오는 데이터 패킷을 분석하여 브리지가 주어진 패킷을 다른 네트워크로 전송할 수 있는지를 결정
3. 허브: OSI L2 스위치로 LAN 환경에서 장치 간 연결을 시켜준다.
