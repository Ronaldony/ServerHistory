...
## Section06 조건변수
* 이 섹션은 이해가 덜 됨. 다음 회차에 자세히 읽기
### 유용한 팁과 테크닉
1. 크리티컬 섹션이나 리더-라이더 락같은 락을 사용 시 반드시 사용해야 하거나 사용하지 말아야 할 것이 있다.
2. 지켜야 할 것
    1) 오브젝트 집합당 하나의 락만을 사용하라
        * 논리적으로 단일의 리소스처럼 사용되어야 하는 리소스들을 읽거나 쓰는 경우 하나의 락만을 유지해야 한다.
        * 단일 리소스의 일부에 접근하더라도 하나의 락을 통해 접근해야 한다.
    2) 다수의 논리적 리소스들에 동시에 접근하는 방법
        * 다수의 리소스에 락을 설정하는 순서를 항상 동일하게 유지하도록 코드를 작성해야 한다.
            <pre><code>
            Func1()
                EnterCriticalSection(&rsc1);            
                EnterCriticalSection(&rsc2);
                
            Func2()
                EnterCriticalSection(&rsc1);            
                EnterCriticalSection(&rsc2);
            * 두 함수의 락 설정(EnterCriticalSection 함수) 순서가 동일 해야함
            </code></pre>
    3) 락을 장시간 점유하지 마라
