...
2. GetQueuedCompletionStatus(GQCS)
    ...
    2) 구조 
        ...
        * FALSE 반환: 디큐잉을 실패한 경우와 작업이 실패한 경우에 FALSE가 반환된다. 중간에 연결이 끊겼을 때는 lpNumberOfBytesTransferred가 0 이어도 TRUE를 반환한다.
        * 세션이 끊긴 상황: 실제 테스트에서는 세션 연결이 끊겼을 때 FALSE가 반환되고 lpOverlapped는 NULL이 아니었다.
    3) 인자 정보
        * lpCompletionKey: 소켓이 대상이라면, 어떤 대상-세션을 찾기 위한 수단으로 사용
        * lpOverlapped: 소켓이 대상이라면, Send냐 Recv냐를 판단할 수 있는 수단으로 사용
    4) 반환 값
        * TRUE: FALSE 그 외 상황. (세션의 연결이 끊겨 lpNumberOfBytesTransferred가 0이 나와도 TRUE를 반환함)
